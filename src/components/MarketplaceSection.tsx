      ...
      <div className="max-w-[1200px] mx-auto" ref={ref}>
        ...
        <div className="flex gap-4 flex-wrap my-10 items-center">
          ...
          {FILTERS.map((f) => (
            ...
            >
              <button
                className={`btn-sweep relative overflow-hidden px-5 py-2 font-mono text-[.65rem] tracking-[.1em] uppercase cursor-none border transition-all duration-300
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
          {CARDS.map((card, i) => (
            <div
              key={i}
              className={`tilt-card mkt-card bg-background p-8 relative overflow-hidden cursor-none transition-all duration-600 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
            ...
            >
              ...
              <h3 className="text-base font-bold mb-2 leading-tight relative z-[2]">{card.title}</h3>
              <p className="font-mono text-[.7rem] leading-[1.7] text-foreground/40 line-clamp-3 relative z-[2]">{card.preview}</p>
              <div className="flex justify-between items-center mt-5 pt-4 border-t border-border relative z-[2]">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <div className="mag-wrap" onMouseMove={handleMagMove} onMouseLeave={handleMagLeave}>
                    <button className="btn-sweep relative overflow-hidden border border-border text-foreground/35 font-mono text-[.62rem] tracking-[.08em] px-3 py-1 cursor-none transition-all duration-300 uppercase hover:border-primary hover:text-primary">
                      Fork
                    </button>
                  </div>
                </div>
